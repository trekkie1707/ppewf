	...
	def readFile(self, c, off, leng, byt, retPos ):
		...
		while( cnt > 0 and leng > 0 and not len(byt) == retPos):
			...
			if(flags[0] == 0):
				...
				f.readinto(arr);
				f.seek(old);
				unkb = bytearray(zlib.decompress(arr));
				tmp = len(byt)-retPos;
				if(tmp <= len(unkb)-myStart):
					byt[retPos:retPos+tmp] = unkb[myStart:myStart+tmp];
					retPos = retPos + tmp;
				else:
					byt[retPos:retPos+(len(unkb)-myStart)] = unkb[myStart:len(unkb)];
				...
				if(tmp <= len(arr)-myStart):
					byt[retPos:retPos+tmp] = arr[myStart:myStart+tmp];
					retPos = retPos + tmp;
				else:
					byt[retPos:retPos+(len(arr)-myStart)] = arr[myStart:len(arr)];
				...
				if(tmp <= len(stdBuff)-myStart):
					byt[retPos:retPos+tmp] = stdBuff[myStart:myStart+tmp];
					retPos = retPos + tmp;
				else:
					byt[retPos:retPos+(len(stdBuff)-myStart)] = stdBuff[myStart:len(stdBuff)];
	...
	def readFile(self, c, off, leng, byt, retPos ):
		...
		while( cnt > 0 and leng > 0 and not len(byt) == retPos):
			...
			if( a < 0):
				...
				#print(f.tell());
				if(not last == by):
					unkb = bytearray(zlib.decompress(by));
					last[:] = by;
					retArray[:] = unkb;
				else:
					unkb = retArray;
				...
				if(tmp <=self.byperchunk):
					byt[retPos:retPos+(tmp-myStart)] = unkb[myStart:myStart+(tmp-myStart)];
					if(tmp-myStart < 0):
						print('blah1!');
						input();
					retPos += (tmp-myStart);
					...
					byt[retPos:retPos+(self.byperchunk-myStart)] = unkb[myStart:myStart+(self.byperchunk-myStart)];
				...
				if(tmp <= self.byperchunk):
					...
					byt[retPos:retPos+(tmp-myStart)] = stdBuff[myStart:myStart+(tmp-myStart)]
					if(tmp-myStart < 0):
						print('blah3!');
						input();
					retPos += (tmp-myStart);
					...
					byt[retPos:retPos+(self.byperchunk-myStart)] = stdBuff[myStart:myStart+(self.byperchunk-myStart)];
					if(self.byperchunk-myStart < 0):
		...
		while( cnt > 0 and leng > 0 and not len(byt) == retPos):
			...
			if( a < 0):
				...
				if(tmp < len(unkb)):
					...
					byt[retPos:retPos+(len(unkb)-myStart)] = unkb[myStart:myStart+(len(unkb)-myStart)];
				...
				if (len(newBuff) < self.byperchunk):
					byt[retPos:retPos+(len(newBuff)-4)] = newBuff[myStart:myStart+(len(newBuff)-4)];
					retPos += len(newBuff)-4;
					break;
				if( tmp < self.byperchunk):
					byt[retPos:retPos+(tmp - myStart)] = newBuff[myStart:myStart+(tmp-myStart)];
					retPos += (tmp-myStart);
					leng = (tmp-myStart);
				else:
					byt[retPos:retPos+(self.byperchunk-myStart)] = newBuff[myStart:myStart+(self.byperchunk-myStart)];
					retPos += self.byperchunk - myStart;
					leng += self.byperchunk - myStart;
			cnt -= 1;
			myStart = 0;
